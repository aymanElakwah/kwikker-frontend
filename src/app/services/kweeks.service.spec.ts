...
describe("kweekService", () => {
    ...
    KWEEKS = [
      {
        ...
        user: {
          ...
          profile_image_url:
            "https://pbs.twimg.com/profile_images/1030601937115381760/tYVfVdN__400x400.jpg",
          following: true,
          follows_you: true,
          blocked: false,
          muted: false
  ...
  describe("injectTagsInText", () => {
    it("should inject the tags correctly", () => {
      ...
      let toBeStr =
        "hello world " +
        hashtagStartTagOpen +
        "/search/people?filterBy=#abdulrahman_khalid&src=hash" +
        startTagClose +
        "#abdulrahman_khalid" +
        endTag +
        " abdulrahman " +
      ...
      toBeStr =
        ...
        " " +
        hashtagStartTagOpen +
        "/search/people?filterBy=#c&src=hash" +
        startTagClose +
        "#c" +
        endTag +
        " d d " +
        hashtagStartTagOpen +
        "/search/people?filterBy=#e&src=hash" +
        startTagClose +
        "#e" +
        endTag +
        " " +
  ...
  describe("like function", () => {
    it("should call likeKweek from data service and like kweek", () => {
      ...
      let spy = spyOn(dataService, 'likeKweek').and.returnValue({ subscribe: () => {
        kWK_ARR[0].liked_by_user = true;
        kWK_ARR[0].number_of_likes++;
      }});
  ...
  describe("unlike function", () => {
    it("should call unlikeKweek from data service and unlike kweek", () => {
      ...
      let spy = spyOn(dataService, 'unlikeKweek').and.returnValue({ subscribe: () => {
        kWK_ARR[0].liked_by_user = false;
        kWK_ARR[0].number_of_likes--;
      }});
  ...
  describe("rekweek function", () => {
    it("should call rekweekKweek from data service and rekweek kweek", () => {
      ...
      let spy = spyOn(dataService, 'rekweekKweek').and.returnValue({ subscribe: () => {
  ...
  describe("unrekweek function", () => {
    it("should call unrekweekKweek from data service and unrekweek kweek", () => {
      ...
      let spy = spyOn(dataService, 'unrekweekKweek').and.returnValue({ subscribe: () => {
